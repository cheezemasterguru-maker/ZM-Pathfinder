(function () {
  ...
  function getPathEndpointPoint(path, role, cell, pad, topPad, rowOffset) {
    ...
    function boundaryTouch(fromPt, toPt) {
    ...
    const shaftClusters = Array.isArray(w.solveState.shaftClusters) ? w.solveState.shaftClusters : [];
    for (const cluster of shaftClusters) {
      for (const shaftCell of cluster) {
        if (manhattan(last, shaftCell) === 1) {
          const touch = boundaryTouch(last, shaftCell);
    ...
    for (const pt of attackPoints) {
      if (manhattan(last, pt) === 1) {
        const touch = boundaryTouch(last, pt);
        if (touch) return touch;
      }
    }
  ...
  function drawPath(ctx, path, color, width, cell, pad, topPad, rowOffset) {
    ...
    const endPoint = getPathEndpointPoint(path, isBlue ? "blue" : "red", cell, pad, topPad, rowOffset);
    if (
      endPoint &&
      (endPoint.x !== points[points.length - 1].x || endPoint.y !== points[points.length - 1].y)
    ) {
