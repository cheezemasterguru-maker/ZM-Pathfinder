window.ZM_HELP = {
  ...
  modalHelp: `
    ...
    <div class="help-section">
      ...
      <p>You can either build the board manually or load a stored map, if that feature is available to your account.</p>
    </div>

    <div class="help-section">
      <h3>Map Loader</h3>
      <p><b>Map Loader</b> is currently visible only to the administrator while maps are being built and tested.</p>
      <p>Use the dropdowns in order:</p>
      <p><b>Event Type</b> → choose Main or Legacy.</p>
      <p><b>Event Name</b> → choose the mine or event name.</p>
      <p><b>Event Mine</b> → this appears only for Legacy events.</p>
      <p><b>Event Chamber</b> → choose the chamber or graveyard for the selected path.</p>
      <p>The next menu will not appear until the previous selection is made.</p>
      <p>Once a valid chamber is selected, the <b>Load Map</b> button appears. Press it to place the stored board into the grid.</p>
      <p>The title can update automatically from the selected map path.</p>
    </div>

    <div class="help-section">
      <h3>How manual input works</h3>
      <p><b>Number:</b> Tap a cell and type the mineral value for that position.</p>
      <p><b>Paste From Clipboard:</b> Select the starting cell first, then use Paste From Clipboard to place copied values from Google Sheets or Excel.</p>
      <p>Blank pasted cells do not erase existing content.</p>
    </div>
    ...
    <div class="help-section">
      ...
      <p>The report summarizes the selected solution and current solver version.</p>
    </div>
