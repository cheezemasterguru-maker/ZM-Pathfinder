(function () {
  console.log("ZM Solver V5.5 loaded");

  const SOLVER_VERSION = "V5.5";
  ...
  function cellWeight(grid, r, c, freeCells, options = {}) {
    ...
    if (v === "") {
      return priorities.blankFlat;
    }

    if (typeof v === "number") {
      let cost = numberCost(v) * priorities.mineralMultiplier + priorities.mineralFlat;
  ...
  function firstBubbleTravelCost(path, grid, objectPriorities) {
    ...
    for (let i = 1; i <= idx; i++) {
      const [r, c] = path[i];
      total += cellWeight(grid, r, c, new Set(), { objectPriorities });
    }
    return total;
  }
